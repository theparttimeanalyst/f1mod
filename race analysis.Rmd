...
ALB,C4,C2
STR,C4,C3
", sep = ",", header = FALSE, fill = TRUE, col.names = c("Driver", "Stint 1", "Stint 2", "Stint 3", "Stint 4"))




saud_strat = data %>%   pivot_longer(cols = 2:5, 
               names_to = "stin", 
               values_to = "tyre")  %>%
                  group_by(Driver) %>% 
                  mutate(stint = 1:n()) %>%
...
austria_gp_2023 <- data.frame(
  Driver = c("VER", "LEC", "PER", "SAI", "NOR", "ALO", "HAM", "RUS", "GAS", "STR", "ALB", "OCO", "SAR", "ZHO", "DEV", "BOT", "PIA", "TSU", "MAG", "HUL"),
  
  Start = c("C4", "C4", "C4", "C4", "C4", "C3", "C4", "C4", "C4", "C4", "C4", "C4", "C4", "C4", "C4", "C3", "C4", "C4", "C3", "C4"),
  Stint_2 = c("C3", "C4", "C4", "C4", "C3", "C4", "C3", "C3", "C3", "C3", "C3", "C3", "C3", "C3", "C3", "C4", "C3", "C3", "C4", "C3"),
  Stint_3 = c("C4", "C3", "C3", "C3", "C3", "C4", "C4", "C4", "C4", "C3", "C3", "C3", "C3", "C3", "C4", "C3", "C3", "C3", "C3", NA),
  Stint_4 = c("C5", NA, NA, NA, NA, NA, NA, NA, NA, "C4", NA, NA, NA, NA, NA, NA, "C4", "C4", "C3", NA)
)




au_strat = austria_gp_2023 %>%   pivot_longer(cols = 2:5, 
               names_to = "stin", 
               values_to = "tyre")  %>%
                  group_by(Driver) %>% 
                  mutate(stint = 1:n()+1) %>%
...
brit_gp_2023 <- data.frame(
Driver = c("VER","NOR", "HAM","PIA","RUS","PER","ALO","ALB","LEC","SAI","SAR","BOT","HUL","STR","ZHO","TSU","DEV", "GAS", "MAG", "OCO"),
  
  Start = c("C2", "C2", "C2", "C2", "C3", "C2", "C2", "C2", "C2", "C2", "C2", "C1", "C1", "C2", "C2", "C3", "C3", "C2", "C2", "C3"),
Stint_2 = c("C3", "C1", "C3", "C1", "C2", "C3", "C3", "C3", "C1", "C1", "C3", "C3", "C2", "C3", "C1", "C1", "C2", "C3", "C4", "C3"),
Stint_3 = c(NA, NA, NA, NA, NA, NA, NA, NA, "C2", NA, NA, NA, "C3", NA, "C3", "C3", "C3", NA, NA, NA)
)




gb_strat = brit_gp_2023 %>%   pivot_longer(cols = 2:4, 
               names_to = "stin", 
               values_to = "tyre")  %>%
                  group_by(Driver) %>% 
                  mutate(stint = 1:n()+1) %>%
...
all_race2 = all_race %>% left_join(all_race_ms, by = c("lap", "code", "stint")) %>%
   ...
   select(raceId, driverId, lap, position, seconds, year, name, round, code, stint, lap5) %>%
    left_join(strat_sum, by = c("code", "stint", "raceId")) %>%
...
min_stint = all_race2 %>% group_by(raceId, code, stint) %>%
                            slice_min(lap5, with_ties = F) %>%
                              rename(minla = lap5) %>%
                                    ungroup() %>%
                                  select(raceId, driverId, stint, minla)




min_lap4 = all_race2 %>% group_by(raceId, lap) %>%
                            slice_min(lap5, with_ties = F) %>%
                                rename(flap = lap5) %>%
                                    ungroup() %>%
                                  select(raceId, lap, flap)



all_rac3 = all_race2 %>%
                        left_join(min_stint, by = c("raceId", "driverId", "stint")) %>%
                          left_join(min_lap4, by = c("raceId", "lap"))
...
dri_team = results %>% select(raceId, driverId, constructorId) %>%
                          left_join(races, by = "raceId") %>%
                    left_join(drivers, by = "driverId") %>%
                      left_join(constructors, by = "constructorId") %>%
                            select(year, code, name.y) %>%
                                distinct()
...
race_sum = all_rac3 %>%
                        filter(raceId == 1108) %>%
                            mutate(perloss = lap5/minla-1) %>%
                                filter(tyre != "C5")




ggplot(race_sum, aes(x = sitntl, y = perloss, col = tyre)) + geom_point() + # + facet_wrap(~tyre)
                                                                geom_smooth(se = F)
...
verstapp = all_rac3 %>% filter(code == "NOR") %>%
                              filter(raceId == 1108) %>%
                               #   filter(tyre == "C4") %>%
                                          mutate(perloss = lap5/minla-1) 
...
ggplot(verstapp, aes(x = sitntl, y = lap5, col = as.factor(stint))) + geom_point()
...
RACES_2022 = all_rac3 %>% filter(year == 2022) %>%
                                  group_by(round, name) %>%
                                    slice_min(lap5, with_ties = F) %>%
                                      rename(prevf = lap5) %>%
                                        select(name, prevf)
...
pace_comp = all_rac3 %>% filter(year == 2023) %>%
                    left_join(RACES_2022, by = "name") %>%
                                left_join(dri_team, by = c("code", "year")) %>%
                                    filter(round.x != 7) %>%
                                  mutate(per = lap5/prevf-1) %>%
                                    group_by(name.y, round.x) %>%
...
ggplot(pace_comp, aes(x = round.x, y = meanper, col = name.y)) + geom_smooth(method = "lm", se = F)
...
seas23 = all_rac3 %>% left_join(dri_team, by = c("code", "year")) %>%
                              mutate(lapdelt = lap5/flap-1) %>%
                                filter(year == 2023) %>%
                                  group_by(name, raceId, name.y) %>%
                                      summarise(meanper = mean(lapdelt)) %>%
                                            rename(delt23 = meanper)




seas22 = all_rac3 %>% left_join(dri_team, by = c("code", "year")) %>%
                              mutate(lapdelt = lap5/flap-1) %>%
                                filter(year == 2022) %>%
                       mutate(nam2 = if_else(name == "Brazillian Grand Prix", "São Paulo Grand Prix", name)) %>%
                                  select(-name) %>%
                                    rename(name = nam2) %>%
                                  group_by(name, raceId, name.y) %>%
                                      summarise(meanper = mean(lapdelt)) %>%
                                            rename(delt22 = meanper)




seascomp = seas23 %>% left_join(seas22, by = c("name.y", "name"))



ggplot(seascomp, aes(x = delt22, y = delt23)) + geom_point() + geom_smooth(method = "lm") + facet_wrap(~name.y, scales = "free")
...
season23 = races %>% filter(year == 2023)


season_mod = function(x) {


teammod = seascomp %>% filter(name.y == x)

prev_sea = seas22 %>% filter(name.y == x)


seas_lm = lm(delt23 ~ delt22, teammod)


preds = predict(seas_lm, prev_sea, interval = "predict")


prev_sea_pred = prev_sea %>% bind_cols(preds) %>%
                                  left_join(seas23, by = c("name.y", "name")) %>%
                                    mutate(delta = if_else(is.na(delt23), fit, delt23))


prev_sea_pred2 = season23 %>% left_join(prev_sea_pred, by = "name") %>%
                                    mutate(sourn = if_else(is.na(delt23), "pred", "act")) %>%
                                      select(round, name, delta, sourn) %>%
                                      mutate(team = x)

return(prev_sea_pred2)
                                

}


teams = c("Alfa Romeo", "AlphaTauri", "Alpine F1 Team", "Aston Martin", "Ferrari", "Haas F1 Team", "McLaren", "Mercedes", "Red Bull", "Williams")



all_team_pred = map_dfr(teams, season_mod)
...
test_au = pace_comp %>% filter(raceId == 1108) %>%
                              group_by(lap) %>%
                                slice_min(lap5) %>%
                                    select(lap, lap5, prevf) %>%
                                       pivot_longer(cols = 2:3, names_to = "year", values_to = "laptime")



ggplot(test_au, aes(x = lap, y = laptime, col = year)) + geom_point()
...
aus = laptimes %>% filter(raceId == 1107) %>%
                            left_join(races, by = "raceId") %>%
                                left_join(drivers, by = "driverId") %>%
                        left_join(pits, by = c("raceId", "lap", "driverId")) %>%
                            left_join(lapcount, by = "raceId") %>%
...
ggplot(aus, aes(x = lap, y = seconds)) + geom_point()



aus2 = laptimes %>% filter(raceId == 1095) %>%
                            left_join(races, by = "raceId") %>%
                                left_join(drivers, by = "driverId") %>%
                        left_join(pits, by = c("raceId", "lap", "driverId")) %>%
                            left_join(lapcount, by = "raceId") %>%
                               mutate(seconds = milliseconds.x / 1000) %>%
                                  mutate(laprem = tlap-lap) %>%
                                    mutate(fuelimp = 0.06 *laprem) %>%
                                      mutate(fuelcorlap = seconds-fuelimp) %>%
                                          filter(fuelcorlap < 80)


ggplot(aus2, aes(x = lap, y = fuelcorlap)) + geom_point()
...
test = min_lap4 %>% group_by(raceId, lap) %>%
                          summarise(n = n()) %>%
                            filter(n > 1)
